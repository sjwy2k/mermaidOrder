<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
<c:set var="path" value="${pageContext.request.contextPath }" />
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title>머메이드오더</title>
		<meta property="og:type" content="website">
		<meta property="og:title" content="머메이드오더 관리자">
		<meta name="description" content="쉽고 빠른 주문. 모든 매장용 사이렌오더">
		<meta property="og:description" content="쉽고 빠른 주문. 모든 매장용 사이렌오더">
		<meta property="og:image" content="${path}/resources/img/mermaid.png" alt="로고영역">

		<!-- Latest compiled and minified CSS -->
		<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css">
		<!-- jQuery library -->
		<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
		<!-- Popper JS -->
		<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
		<!-- Latest compiled JavaScript -->
		<script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js"></script>

		<!-- favicon.ico -->
		<meta property="og:url" content="${path}/resources/">
		<link rel="apple-touch-icon" sizes="180x180" href="${path}/resources/img/mermaid.png">
		<link rel="icon" type="image/png" sizes="32x32" href="${path}/resources/img/mermaid.png">
		<link rel="icon" type="image/png" sizes="16x16" href="${path}/resources/img/mermaid.png">

		<script src="${path}/resources//js/native.js"></script>
		<link rel="stylesheet" href="${path}/resources/css/styles-order.css">
		
		<!-- css import start -->
		<style>
			@font-face {
				font-family: 'system_font';
				src: local('Nanum Barun Gothic'),
					url('/index/fonts/NanumBarunGothic/ultraLight.woff2') format('woff2'),
					font-weight: 200;
				font-style: normal;
				font-display: swap;
			}

			@font-face {
				font-family: 'system_font';
				src: local('Nanum Barun Gothic'),
					url('/index/fonts/NanumBarunGothic/light.woff2') format('woff2'),
					font-weight: 300;
				font-style: normal;
				font-display: swap;
			}

			@font-face {
				font-family: 'system_font';
				src: local('Nanum Barun Gothic'),
					url('/index/fonts/NanumBarunGothic/regular.woff2') format('woff2'),
					font-weight: 400;
				font-style: normal;
				font-display: swap;
			}

			@font-face {
				font-family: 'system_font';
				src: local('Nanum Barun Gothic'),
					url('/index/fonts/NanumBarunGothic/bold.woff2') format('woff2'),
					font-weight: 700;
				font-style: normal;
				font-display: swap;
			}
		</style>
		<style type="text/css">
			body.imp-payment-progress {
				position: static
			}

			body.imp-payment-progress>:not (.imp-dialog) {
				display: none
			}

			.imp-dialog {
				display: none;
				position: fixed;
				top: 0;
				bottom: 0;
				left: 0;
				right: 0;
				width: 100%;
				height: 100%;
				z-index: 99999;
			}

			.imp-dialog .imp-frame-pc.imp-frame-danal,
			.imp-dialog .imp-frame-pc.imp-frame-danal_tpay {
				left: 50% 
				margin-left: -345px;
				width: 700px 
				height: 700px 
				margin-top: 50px;
			}

			.imp-dialog .imp-header {
				display: none;
				background: transparent;
				position: absolute;
				top: 0;
				left: 0;
				right: 0;
				height: 25px;
			}

			.imp-dialog .imp-close {
				text-decoration: none;
				position: absolute;
				top: 10px;
				right: 10px;
				cursor: pointer;
				background:
					url('data:image/png;base64,');
			}

			.imp-dialog.pc .imp-frame-danal-certification {
				width: 410px 
				height: 660px 
				margin: 10px auto;
				background: #fff;
			}

			.imp-dialog.pc.certification-danal {
				background: rgb(255, 255, 255);
				background: rgba(0, 0, 0, 0.5);
			}

			.imp-dialog.pc.certification-danal .imp-header {
				display: block;
				width: 410px;
				margin: 0 auto;
			}

			.imp-dialog.pc.certification-danal .imp-header .imp-close {
				top: 18px;
				right: 25px;
				width: 19px;
				height: 19px;
			}

			.imp-dialog.mobile.ios {
				position: absolute;
			}

			.imp-dialog.mobile.certification-danal .imp-header {
				display: block;
			}

			.imp-dialog.mobile.certification-danal .imp-header .imp-close {
				top: 6px;
				right: 10px;
				width: 19px;
				height: 19px;
			}

			.imp-dialog.pc.payment-kakaopay {
				background: rgb(221, 221, 221);
				background: rgba(0, 0, 0, 0.5);
			}

			.imp-dialog.pc.payment-kakaopay .imp-frame-kakaopay {
				width: 426px 
				height: 550px 
				left: 50% 
				top: 50% 
				margin-left: -213px 
				margin-top: -275px 
			}
		</style>
		<style type="text/css">
			/* Copyright 2014-present Evernote Corporation. All rights reserved. */
			@ keyframes caretBlink {
				from {
					opacity: 1.0;

				}

				to {
					opacity: 0.0;
				}

			}

			@ keyframes rotateSpinner {
				from {
					transform: rotate(0deg);

				}

				to {
					transform: rotate(360deg);
				}

			}

			#text-tool-caret {
				animation-name: caretBlink;
				animation-iteration-count: infinite;
				animation-timing-function: cubic-bezier(1.0, 0, 0, 1.0);
				animation-duration: 1s;
			}

			#en-markup-loading-spinner {
				position: absolute;
				top: calc(50% - 16px);
				left: calc(50% - 16px);
				width: 32px;
				height: 32px;
			}

			#en-markup-loading-spinner img {
				position: relative;
				top: 0px;
				left: 0px;
				animation-name: rotateSpinner;
				animation-duration: 0.6s;
				animation-iteration-count: infinite;
				animation-timing-function: linear;
			}
		</style>
		<style type="text/css">
			/* Copyright 2014-present Evernote Corporation. All rights reserved. */
			.skitchToastBoxContainer {
				position: absolute;
				width: 100%;
				text-align: center;
				top: 30px;
				-webkit-user-select: none;
				-moz-user-select: none;
				pointer-events: none;
			}

			.skitchToastBox {
				width: 200px;
				height: 16px;
				padding: 12px;
				background-color: rgba(47, 55, 61, 0.95);
				border-radius: 4px;
				color: white;
				cursor: default;
				font-size: 10pt;
				text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.32);
				font-family: 'Soleil', Helvetica, Arial, sans-serif;
				border: 2px rgba(255, 255, 255, 0.38) solid;
			}

			.lang-zh-cn .skitchToastBox {
				font-family: '微软雅黑', 'Microsoft YaHei', SimSun,
					'&#x30E1;&#x30A4;&#x30EA;&#x30AA;', Meiryo, 'MS PGothic', 'Soleil',
					Helvetica, Arial, sans-serif;
			}

			.lang-ja-jp .skitchToastBox {
				font-family: '&#x30E1;&#x30A4;&#x30EA;&#x30AA;', Meiryo, 'MS PGothic',
					'微软雅黑', 'Microsoft YaHei', SimSun, 'Soleil', Helvetica, Arial,
					sans-serif;
			}

			.skitchToast {
				padding-left: 20px;
				padding-right: 20px;
				display: inline-block;
				height: 10px;
				color: #f1f5f8;
				text-align: center;
			}

			.skitchVisible {
				/* Don't remove this class it's a hack used by the Evernote Clipper */

			}
		</style>
		<style type="text/css">
			/* Copyright 2014-present Evernote Corporation. All rights reserved. */
			@font-face {
				font-family: 'Soleil';
				src:
					url(data:application/font-woff2;base64,);
				font-weight: normal;
				font-style: normal;
			}
		</style>
		<style type="text/css">
			/* Copyright 2019-present Evernote Corporation. All rights reserved. */
			#en-markup-disabled {
				position: fixed;
				z-index: 9999;
				width: 100%;
				height: 100%;
				top: 0px;
				left: 0px;
				cursor: default;
				-webkit-user-select: none;
			}

			#en-markup-alert-container {
				position: absolute;
				z-index: 9999;
				width: 450px;
				left: calc(50% - 225px);
				top: calc(50% - 85px);
				background-color: white;
				box-shadow: 0 2px 7px 1px rgba(0, 0, 0, 0.35);
				-webkit-user-select: none;
			}

			#en-markup-alert-container .cell-1 {
				position: relative;
				height: 110px;
				width: 105px;
				float: left;
				text-align: center;
				background-image:
					url(data:image/png;base64,);
				background-position: 65% 50%;
				background-repeat: no-repeat;
			}

			#en-markup-alert-container .cell-2 {
				position: relative;
				float: left;
				width: 345px;
				margin-top: 29px;
				margin-bottom: 20px;
			}

			#en-markup-alert-container .cell-2 .cell-2-title {
				margin-bottom: 5px;
				padding-right: 30px;
				font-size: 12pt;
				font-family: Tahoma, Arial;
			}

			#en-markup-alert-container .cell-2 .cell-2-message {
				padding-right: 30px;
				font-size: 9.5pt;
				font-family: Tahoma, Arial;
			}

			#en-markup-alert-container .cell-3 {
				position: relative;
				width: 450px;
				height: 60px;
				float: left;
				background-color: rgb(240, 240, 240);
			}

			#en-markup-alert-container .cell-3 button {
				position: absolute;
				top: 12px;
				right: 15px;
				width: 110px;
				height: 36px;
			}

			#en-markup-alert-container .cell-3 button.alt-button {
				position: absolute;
				top: 12px;
				right: 140px;
				width: 110px;
				height: 36px;
			}
		</style>
		<style>
			.instruction .component {
				color: #fff;
				margin-left: 5px;
				margin-top: 37px;
				position: absolute;
				font-size: 16px;
				font-weight: 300
			}

			.settings .component {
				float: right;
				position: relative;
				top: 17.5px;
				right: 17px;
				width: 44px 
				height: 44px 
				border: none;
				outline: 0;
				padding: 8px;
				background-color: transparent
			}

			.settings .component * .component {
				width: 100%;
				height: 100%
			}

			.menu-cell .component {
				position: relative;
				float: left;
				width: 33.33%;
				height: 151px
			}

			.menu-cell .component * .component {
				position: absolute;
				left: 0;
				right: 0;
				margin: auto
			}

			.menu-cell .component .menu-image-border .component {
				top: 8px;
				width: 79px;
				height: 79px;
				border-radius: 79px;
				border: 1px solid #a9afb9
			}

			.menu-cell .component .menu-image .component {
				top: 9px;
				width: 79px;
				height: 79px;
				-o-object-fit: cover;
				object-fit: cover;
				border-radius: 79px
			}

			.menu-cell .component .menu-text .component {
				top: 91px;
				width: 89px;
				font-size: 16px;
				font-weight: 500;
				text-align: center;
				line-height: 20px;
				max-height: 20px
			}

			.menu-cell .component .menu-text .component * .component {
				position: relative
			}

			.menu-cell .component .menu-text .component .menu-name .component {
				max-height: 40px;
				overflow: hidden
			}

			.menu-cell .component .menu-text .component .menu-price .component {
				font-size: 14px;
				font-weight: 300
			}

			.menu-cell .component .menu-count .component {
				top: 8px;
				left: 50% 
				margin: 0;
				margin-left: 18px 
				width: 17px;
				height: 17px;
				border-radius: 8.5px;
				background-color: #2489ff;
				color: #fff 
				text-align: center;
				font-size: 11px;
				font-weight: 700;
				line-height: 19px;
				display: none
			}

			.menu-cell.selected .component * .component {
				color: #2489ff
			}

			.menu-cell.selected .component .menu-count .component {
				display: block
			}

			.menu-cell.selected .component .menu-image-border .component {
				top: 6px;
				width: 81px;
				height: 81px;
				border-radius: 81px;
				border: 2px solid #2489ff
			}

			.header .component .logo .component {
				position: relative;
				top: 3px;
				left: 18px;
				width: 81px;
				height: 43px
			}

			.header .component .mypage .component {
				position: relative;
				top: 5px;
				right: 5px;
				width: 46px;
				height: 46px;
				padding: 8px;
				border: none;
				outline: 0;
				background-color: transparent;
				float: right
			}

			.header .component .mypage .component img .component {
				width: 100%;
				height: 100%
			}

			.stories.component {
				padding-top:10px;
			}

			.story.component {
				height: 400px;
				overflow: hidden;
				margin: 2% 0 3% 4%;
				padding-top: 12px;
				box-shadow: 0 4px 10px 4px rgba(0, 0, 0, .15);
				border-radius: 13px
			}

			.story.menu.single-column .component {
				height: auto;
				max-height: 400px
			}

			.story .component .subtitle .component,
			.story .component .tag .component,
			.story .component .title .component {
				color: #04A967;
				overflow: hidden;
				margin: auto 18px;
				text-overflow: ellipsis;
				white-space: nowrap
			}

			.story .component .tag .component {
				margin-bottom: 2px;
				font-size: 14px;
				line-height: 16px;
				height: 22px;
				padding: 3px 17px 0;
				display: inline-block;
				box-sizing: border-box;
				border-radius: 11px;
				border: 1px solid #04A967
			}

			.story.component .title.component {
				font-weight: 700;
				margin-top: 5px;
				margin-bottom: 14px;
				height: 41px;
				font-size: 22px;
				line-height: 41px
			}

			.story .component .subtitle .component {
				text-align: right;
				padding-right: 10px;
				font-size: 18px;
				line-height: inherit;
				font-weight: 600;
				color: #786253
			}

			.story.cover .component .title .component {
				margin-bottom: 0;
				color: #786253
			}

			.story .component .cover .component {
				margin: -18px 0 10px;
				width: 100%;
				height: 294px;
				-o-object-fit: cover;
				object-fit: cover;
				border-top-left-radius: 13px;
				border-top-right-radius: 13px
			}

			.story.component .show-more{
				position: relative;
				height: 35px
			}

			.story.component .show-more .text {
				position: absolute;
				width: 100%;
				text-align: center;
				font-size: 13px;
				line-height: 27px;
				font-weight: 700;
				color: #786253
			}

			.company .component {
				margin-top: 26px;
				padding-bottom: 50px;
				background-color: #04A967
			}

			.company .component .name .component {
				display: inline-block;
				padding: 0 0 0 29px;
				margin: 15px 0 10px;
				color: #fff
			}

			.company .component .close_more .component,
			.company .component .more .component {
				display: inline-block;
				position: absolute;
				padding: 15px 20px 20px;
				right: 20px;
				font-size: 13px;
				color: #fff
			}

			.company .component .introduce .component {
				font-size: 12px;
				padding: 0 30px;
				margin: 5px 0 0
			}

			.company .component .introduce .component .info .component {
				color: #fff;
				visibility: hidden;
				display: none
			}

			.company .component .introduce .component .info2 .component {
				color: #fff
			}

			.company .component .main_divider .component {
				position: relative;
				bottom: 8px;
				height: .99px;
				background-color: #e0e0e0
			}

			.company .component ul .component {
				list-style: none;
				margin: 0;
				padding: 0;
				font-size: 12px;
				text-align: center
			}

			.company .component li .component {
				margin: 0;
				padding: 0;
				border: 0;
				width: 50%;
				float: left;
				position: relative;
				color: #fff
			}

			.company .component em .component {
				position: absolute;
				top: 0;
				left: 0;
				display: block;
				width: 1px;
				height: 15px;
				background: #e5e5e5
			}

			.company .component a .component {
				color: #fff
			}

			.market-green-point .component {
				height: 30px;
				margin: 0 0 30px;
				background-color: #1ddb16;
				text-align: center
			}

			.market-green-point.happy-hour .component {
				margin: 0 0 5px
			}

			.market-green-point .component .green-point-title .component {
				position: relative;
				top: 6px;
				color: #fff;
				font-size: 15px
			}

			.market-happy-hour component {
				height: 30px;
				margin: 0 0 30px;
				background-color: red;
				text-align: center
			}

			.market-happy-hour .component .happy-hour-time .component {
				position: relative;
				top: 6px;
				color: #fff;
				font-size: 15px
			}

			#draggable .component {
				position: absolute;
				top: 40px;
				right: 25%;
				width: -webkit-fit-content;
				width: -moz-fit-content;
				width: fit-content;
				height: -webkit-fit-content;
				height: -moz-fit-content;
				height: fit-content;
				text-align: center;
				font-size: 12px;
				color: #fff;
				background-color: #008b8b
			}

			#draggable.web-draggable .component {
				top: 10px 
			}
		</style>
		<style>
			.balloon{
				position: relative;
				-webkit-mask-image:
					url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAA5JREFUeNpiYGBgAAgwAAAEAAGbA+oJAAAAAElFTkSuQmCC);
				box-sizing: border-box;
				border-radius: 13px;
				background-color: #fff;
				overflow-x: hidden;
				overflow-y: scroll;
				z-index: 301
			}

			.balloon.alert{
				z-index: 401
			}

			.balloon.alert.app-install{
				background-color: transparent
			}

			.animate{
				-webkit-transform: all .3s ease-out;
				transform: all .3s ease-out;
				transition: all .3s ease-out
			}
		</style>
		<style>
			.status-bar {
				height: 0
			}

			.status-bar.absolute {
				position: absolute
			}

			.background {
				position: fixed;
				top: 0;
				left: 0;
				width: 100%;
				height: 100%;
				z-index: 300;
				background-color: rgba(0, 0, 0, .9)
			}

			.background.alert {
				z-index: 400;
				background-color: rgba(0, 0, 0, .3);
				touch-action: none
			}

			.background.alert.app-install {
				background-color: rgba(17, 10, 5, .79)
			}

			@ supports (-webkit-backdrop-filter:saturate (180%) blur (20px)) {
				. background : not(.alert) {
					background-color: rgba(0, 0, 0, .7);
					-webkit-backdrop-filter: saturate(180%) blur(20px)
				}

			}

			.ios-bottom-keeper {
				position: absolute;
				bottom: 0;
				width: 0;
				background-color: transparent;
				z-index: -100
			}

			.ios-bottom-keeper.development {
				width: 40px;
				height: 40px;
				text-align: center;
				font-size: 24px;
				line-height: 48px;
				z-index: 999;
				color: #fff;
				background-color: red;
				border: 1px solid #fff
			}

			.checkout-button {
				position: fixed;
				right: 17px;
				bottom: 21px;
				width: 110px;
				height: 110px;
				padding: 0;
				border: none;
				outline: 0;
				z-index: 200;
				background: url(checkout.2df9c2093e1ebb005b4b.png) top left/100% 100% no-repeat
			}

			.checkout-button * {
				position: absolute;
				color: #fff;
				font-size: 20px;
				font-weight: 700;
				text-align: center
			}

			.checkout-button .count {
				left: 27px;
				top: 33px;
				min-width: 38px;
				font-size: 40px
			}

			.checkout-button .suffix {
				left: 52px;
				top: 44px;
				width: 30px;
				height: 33px;
				line-height: 33px
			}

			.checkout-button.long .count {
				left: 29px;
				top: 36px;
				font-size: 35px;
				letter-spacing: -1px
			}

			.checkout-button.long .suffix {
				left: 62px;
				top: 44px;
				font-size: 18px
			}

			.alert-close {
				width: 22px;
				height: 22px;
				padding: 10px;
				z-index: 402;
				position: fixed;
				top: 15px;
				right: 14px;
				display: none
			}

			.kiosk-checkout-button {
				position: fixed;
				right: 17px;
				bottom: 21px;
				width: 250px;
				height: 80px;
				padding: 0;
				border: none;
				outline: 0;
				z-index: 200;
				background: url(kioskOrderBtn.34eef1c3912794004560.png) top left/100% 100% no-repeat
			}

			.kiosk-checkout-button * {
				position: absolute;
				color: #ff1818;
				font-size: 20px;
				font-weight: 700;
				text-align: center
			}

			.kiosk-checkout-button .count {
				left: 23px;
				top: 17px;
				width: 38px;
				font-size: 40px;
				font-weight: 400
			}

			.kiosk-checkout-button .suffix {
				left: 48px;
				top: 29px;
				width: 30px;
				height: 33px;
				line-height: 33px;
				font-weight: 400
			}

			.kiosk-checkout-button.long .count {
				left: 23px;
				top: 22px;
				font-size: 32px;
				font-weight: 400
			}

			.kiosk-checkout-button.long .suffix {
				top: 30px;
				left: 54px
			}
		</style>
		<style>
			#alert-identifier {
				display: none
			}
			
			.center {
				padding: 15px 15px 7.5px;
				text-align: center
			}
			
			.center .message, .center .title, .center input {
				margin-bottom: 3.75px
			}
			
			.center .scrollable {
				overflow-y: scroll
			}
			
			.center .title {
				font-weight: 700;
				font-size: 16px
			}
			
			.center .message {
				font-size: 14px
			}
			
			.center input {
				width: 100%;
				border: .5px solid #a9afb9;
				border-radius: 6px;
				box-sizing: border-box;
				-webkit-appearance: none;
				font-size: 14px
			}
			
			.center input:first-of-type {
				margin-top: 3.75px
			}
			
			#dummy {
				position: fixed
			}
			
			button {
				float: left;
				width: 50%;
				margin: 0;
				padding: 12.5px 0;
				border: none;
				border-top: .5px solid #a9afb9;
				border-right: .5px solid #a9afb9;
				text-align: center;
				color: #2489ff;
				font-size: 16px;
				background-color: transparent
			}
			
			button:last-of-type {
				border-right: none
			}
			
			button:active {
				opacity: 1;
				background-color: #f6f6f6
			}
			
			button.cancel {
				font-weight: 700
			}
			
			button.destructive {
				color: #f81c45
			}
			
			button.single {
				width: 100%
			}
			
			.picker-wrapper {
				text-align: center;
				position: relative;
				height: 210px;
				display: inline-block;
				line-height: 30px;
				width: 100%;
				font-size: 20px;
				-webkit-perspective: 1000px;
				perspective: 1000px;
				-webkit-perspective-origin: center center;
				perspective-origin: center center
			}
			
			.picker-wrapper, .picker-wrapper * {
				box-sizing: border-box;
				-webkit-user-select: none
			}
			
			.picker-wrapper .event-scroller {
				position: absolute;
				top: 0;
				width: 100%;
				height: 100%;
				z-index: 5;
				overflow: auto;
				box-shadow: inset 0 10px 24px 3px #fff, inset 0 -20px 24px 3px #fff
			}
			
			.picker-wrapper .event-scroller .option {
				visibility: hidden;
				color: red;
				opacity: .2
			}
			
			.picker-wrapper .bottom-line, .picker-wrapper .top-line {
				position: absolute;
				z-index: 2;
				background: rgba(255, 255, 255, .6);
				width: 100%;
				border-bottom: 1px solid #d3d3d3
			}
			
			.picker-wrapper .bottom-line {
				border-top: 1px solid #d3d3d3;
				border-bottom: none
			}
			
			.picker-wrapper .view-scroller {
				height: 100%;
				-webkit-transition: -webkit-transform .1s;
				-webkit-transform: translateZ(-90px) rotateX(0);
				-webkit-transform-style: preserve-3d
			}
			
			.picker-wrapper .view-scroller .option {
				position: absolute;
				width: 100%;
				color: #545f74;
				overflow: hidden;
				transition: all .3s;
				-webkit-transform-origin: 50% 50%;
				transform-origin: 50% 50%;
				-webkit-transform-style: preserve-3d;
				backface-visibility: hidden;
				-webkit-backface-visibility: hidden
			}
			
			.app-install {
				text-align: center;
				overflow: scroll
			}
			
			.app-install .logo {
				width: 87px;
				height: 87px;
				vertical-align: top
			}
			
			.app-install .title {
				margin: 14px 0 0 20px;
				display: inline-block;
				font-size: 20px;
				line-height: 29px;
				color: #fff;
				text-align: left
			}
			
			.app-install .install {
				float: none;
				width: 250px;
				height: 53px;
				color: #fff;
				border-radius: 6px;
				text-align: center;
				font-size: 22px;
				font-weight: 700;
				padding: 0;
				line-height: 53px;
				border: none;
				margin-top: 26px;
				background-image: linear-gradient(#ff5151, #ff1818)
			}
			
			.app-install .dismiss {
				margin-top: 24px;
				color: #fff;
				font-size: 18px;
				line-height: 20px;
				text-decoration: underline
			}
			
			.soldout {
				position: absolute;
				top: 0;
				width: 100%;
				opacity: .7
			}
			
			.option-name {
				margin-top: 5px;
				font-size: 15px;
				font-weight: 600;
				height: 22px
			}
			
			.option-name.listed-option-name {
				font-size: 18px;
				margin: 20px 5% 10px 3%
			}
			
			.action.remove {
				margin-left: 15px;
				color: #fff;
				background-color: #f81c45
			}
			.segmented {
				position: relative;
    			display: inline-block;
			}
			.segmented .label {
				display:inline-block;
				height: 35px;
				color: #04A967;
				font-size: 17px;
				background-color: #fff;
				border: .99px solid #04A967;
				box-sizing: border-box;
				border-style: solid;
				cursor: pointer;
				float: left;
				line-height: 33px;
				text-align: center;
				overflow: hidden
			}
			
			.segmented .label .disabled {
				opacity: .6
			}
			
			.segmented:first-child .label {
				border-top-left-radius: 6px;
				border-bottom-left-radius: 6px
			}
			
			.segmented:last-child .label {
				border-top-right-radius: 6px;
				border-bottom-right-radius: 6px;
				border-right-style: solid
			}
			
			.segmented input:checked+.label{
				font-weight: 500;
				background-color: #04A967;
				color: #fff
			}
			
			.segmented input:checked+.label.disabled {
				opacity: .6;
				background-color: #04A967;
				border-color: #04A967
			}
			
			.segmented input {
				display: none
			}
			
			.segmented img.more {
				position: absolute;
				top: 13px;
				right: 10px;
				width: 17px;
				height: 9px;
				z-index: 1
			}
			
			.separator {
				position: relative;
				margin: 0 5% 10px 2%;
				bottom: .55px;
				height: 1px;
				background-color: #04A967
			}
			
			.listed {
				width: 90%;
				margin-left: 3%;
				margin-right: 5%
			}
			
			.listed .option {
				width: 100%;
				line-height: 25px;
				height: 30px;
				padding-bottom: 10px
			}
			
			.listed .option .square {
				width: 26px;
				height: 26px
			}
			
			.listed .option .label {
				font-size: 19px;
				opacity: .9
			}
			
			.listed .option .label.option-value {
				position: relative;
				margin-left: 10px top: 1px
				color: #786253
			}
			
			.listed .option .label.option-price {
				position: absolute;
				right: 5px;
				top: 1px;
				color: #786253
			}
			
			.menu-count {
				position: relative;
				display: block;
				margin: 18px 0 0;
				text-align: center
			}
			
			.menu-count .count-modify {
				display: inline-block;
				color: #786253;
				font-size: 25px;
				font-weight: 400;
				margin: 0 15px;
				text-align: center;
				line-height: 33px
			}
			
			.menu-count .count-image {
				width: 33px;
				height: 33px
			}
			
			.menu-count .count-image:active {
				background-color: #04A967;
				color: #fff;
				border-radius: 19px;
				opacity: .6
			}
		</style>
		<style>
		.market-container .close {
			top: 7px;
			right: 22px
		}
		
		.market-container .item {
			position: relative;
			height: 78px
		}
		
		.market-container .item .img-text {
			display: flex
		}
		
		.market-container .item .img-text .market-image {
			margin: 0 16px 0 24px
		}
		
		.market-container .item .img-text .market-image img {
			vertical-align: top;
			width: 63px;
			height: 63px;
			-o-object-fit: cover;
			object-fit: cover;
			border-radius: 31.5px
		}
		
		.market-container .item .img-text .text {
			display: inline-block;
			margin: 15px 65px 0 0
		}
		
		.market-container .item .img-text .text.outOfBiz {
			margin: 8px 65px 0 0
		}
		
		.market-container .item .img-text .text, .market-container .item .img-text .text *
			{
			color: #04A967;
			text-overflow: ellipsis;
			white-space: nowrap;
			overflow: hidden
		}
		
		.market-container .item .img-text .text .name-text {
			display: flex
		}
		
		.market-container .item .img-text .text .name-text .name {
			flex: 0 1 auto;
			height: 21px;
			color: #4a4a4a;
			font-size: 18px;
			line-height: 20px;
			font-weight: 700;
			display: inline-block
		}
		
		.market-container .item .img-text .text .name-text .market-icon {
			flex: 0 0 auto;
			margin: 2px 0 0 10px;
			width: 15px;
			height: 15px;
			-o-object-fit: cover;
			object-fit: cover;
			border-radius: 7.5px
		}
		
		.market-container .item .img-text .text .desc {
			color: #9b9b9b;
			font-size: 12px;
			font-weight: 300
		}
		
		.market-container .item .pin {
			position: absolute;
			top: 2px;
			right: 17px;
			width: 44px;
			height: 44px;
			padding: 8px;
			border: none;
			outline: 0;
			background-color: transparent
		}
		
		.market-container .item .pin img {
			width: 100%;
			height: 100%
		}
		
		.market-container .item .distance {
			position: absolute;
			top: 25px;
			right: 15px;
			font-size: 14px;
			width: 50px;
			height: 20px;
			background-color: #f9f9f9;
			border-radius: 10px;
			text-align: center;
			color: #a6a5a5;
			line-height: 1.5
		}
		
		.market-container .item .distance.no-border {
			top: 39px;
			color: #ff1818;
			background-color: transparent
		}
		
		.action.remove {
			margin-left: 15px
		}
		
		.action   span {
			color: #fff
		}
		
		.action   img {
			height: 100%
		}
		
		.quota {
			color: #a9afb9;
			font-size: 13px
		}
		
		.list-header {
			margin-top: 30px;
			margin-right: -15px;
			margin-left: -15px;
			padding-left: 15px;
			height: 27px;
			background-color: #2FD085;
			color: #fff;
			font-size: 14px;
			font-weight: 700;
			line-height: 27px
		}
		
		.list-header.no-margin-top {
			margin-top: 0
		}
		
		.list-header.short-margin-top {
			margin-top: 10px
		}
		
		.list-header.short-margin-top-none {
			margin-top: 0 
		}
		
		.market+.list-header {
			margin-top: 22px
		}
		
		.list-cell {
			position: relative;
			margin-left: -15px;
			margin-right: -15px;
			padding: 0 15px;
			height: 72.5px;
			overflow: hidden
		}
		
		.list-cell.kiosk-cell {
			height: auto
		}
		
		.list-cell.coupon-cell {
			height: 85px
		}
		
		.list-cell.short-margin-bottom-none {
			margin-bottom: 0 
		}
		
		.list-cell   [class*=title] {
			margin-right: 110px;
			overflow: hidden;
			white-space: nowrap;
			text-overflow: ellipsis;
			display: flex
		}
		
		.list-cell   [class*=title]   .count, .list-cell   [class*=title]   .name
			{
			overflow: hidden
		}
		
		.list-cell   [class*=title]   .name {
			white-space: nowrap;
			text-overflow: ellipsis;
			flex: 0 1 auto
		}
		
		.list-cell   [class*=title]   .count {
			margin-left: 5px;
			flex: 0 0 auto;
			color: #b5a396;
			font-weight: 600
		}
		
		.list-cell   .title {
			position: relative;
			top: 8px;
			height: 20px;
			margin: 10px 0 0;
			font-size: 17px;
			font-weight: 700;
			line-height: 20px
		}
		
		.list-cell   .title-only {
			position: absolute;
			top: 21px;
			color: #786253;
			font-size: 18px;
			font-weight: 700;
			height: 34px;
			line-height: 34px;
			width: 60%
		}
		
		.list-cell   .title-option {
			position: absolute;
			top: 16px;
			color: #786253;
			font-size: 18px;
			font-weight: 700;
			height: 34px;
			line-height: 34px;
			width: 60%
		}
		
		.list-cell   .name, .list-cell   .price {
			color: #786253
		}
		
		.list-cell   .name {
			font-size: 18px
		}
		
		.list-cell   .price {
			position: absolute;
			top: 21px;
			right: 60px;
			height: 34px;
			font-size: 18px;
			font-weight: 700;
			line-height: 34px
		}
		
		.list-cell   .price.not-selected {
			color: #b5a396
		}
		
		.list-cell   .option {
			position: relative;
			margin-top: 10px;
			margin-right: 110px;
			height: 19px;
			color: #b5a396;
			font-size: 15px;
			font-weight: 300;
			text-overflow: ellipsis;
			overflow: hidden;
			white-space: nowrap
		}
		
		.list-cell   .option.phone-verify-check {
			color: red;
			font-weight: 500
		}
		
		.list-cell   .option.option-margin {
			margin-top: 15px;
			margin-bottom: 10px
		}
		
		.list-cell   .coupon-option {
			position: relative;
			margin: 5px 0 0;
			color: #786253;
			font-size: 12px;
			font-weight: 400;
			text-overflow: ellipsis;
			overflow: hidden;
			white-space: nowrap
		}
		
		.list-cell   .coupon-option.no-option {
			margin-top: 50px
		}
		
		.list-cell   .coupon-option   span {
			color: #786253;
			font-size: 12px;
			font-weight: 400
		}
		
		.list-cell   .accessory {
			position: absolute;
			top: 28.5px;
			right: 15px;
			width: 10px;
			height: 17px
		}
		
		.list-cell   .accessory.square {
			width: 17px
		}
		
		.list-cell   .separator {
			position: absolute;
			left: 15px;
			right: 15px;
			bottom: .55px;
			height: 1px;
			background-color: #b5a396
		}
		
		.list-cell   .menu-remove {
			position: absolute;
			top: 18px;
			right: 5px;
			width: 18px;
			height: 18px;
			padding: 10px
		}
		
		.list-cell   .phone-btn {
			position: absolute;
			top: 17px;
			right: 10px
		}
		
		.list-cell   .phone-btn   .register-btn {
			position: relative;
			width: 85px;
			height: 50px;
			font-size: 17px;
			color: #786253;
			border-radius: 5px;
			background-color: #fff;
			border: .99px solid #786253
		}
		
		.list-cell   .phone-btn   .register-btn:active:enabled {
			background-color: #786253;
			color: #f9f9f9;
			opacity: .6
		}
		
		.list-cell   .phone-btn   .phone-input-btn {
			position: relative;
			width: 85px;
			height: 50px;
			font-size: 17px;
			margin-left: 10px;
			color: #786253;
			border-radius: 5px;
			background-color: #fff;
			border: .99px solid #786253
		}
		
		.list-cell   .phone-btn   .phone-input-btn:active:enabled {
			background-color: #786253;
			color: #f9f9f9;
			opacity: .6
		}
		
		.kiosk-option {
			margin: 10px 0;
			color: #b5a396;
			font-weight: 600;
			font-size: 15px
		}
		
		.kiosk-option   span {
			margin: 10px 0;
			color: #b5a396;
			font-weight: 600
		}
		
		.happy-hour-info {
			position: absolute;
			right: 5px;
			font-size: 13px;
			font-weight: 500;
			margin-top: 0;
			color: #ed0000
		}
		
		.list-notice {
			color: #b5a396;
			font-size: 15px;
			font-weight: 500;
			margin-top: 4px
		}
		
		.list-notice span {
			color: #b5a396;
			font-size: 13px;
			font-weight: 500;
			margin-top: 3.5px
		}
		
		.list-notice   .order-disable {
			color: #f81c45;
			opacity: .9
		}
		
		.list-notice.current-coupon {
			color: #786253;
			font-weight: 400;
			margin: 10px 0
		}
		
		.list-notice.list-margin {
			margin-top: 15px
		}
		
		.list-notice.staffDiscount-info {
			margin-top: 8px
		}
		
		.payment-notice {
			text-align: center;
			font-size: 13px;
			font-weight: 300;
			margin-top: 6px;
			margin-bottom: -21px
		}
		
		.order {
			color: #b5a396;
			text-align: center
		}
		
		.order.name {
			margin-top: 5px;
			margin-right: 0;
			font-size: 30px
		}
		
		.order.status {
			margin-right: 0
		}
		
		.market   .text {
			margin: 28px 0 0;
			height: auto
		}
		
		.market   .text   .desc, .market   .text   .name {
			margin: auto 0
		}
		
		.market   .text   .desc {
			color: #b5a396
		}
		
		.market {
			background-color: transparent;
			height: auto;
			margin: 0
		}
		
		.market   .img-text {
			display: flex
		}
		
		.market   .img-text   img {
			margin: 10px 13px 10px 24px;
			vertical-align: top;
			width: 57px;
			height: 57px;
			-o-object-fit: cover;
			object-fit: cover;
			border-radius: 28.5px
		}
		
		.market   .img-text   .text {
			margin: 28px 0 0;
			height: auto;
			width: 100%
		}
		
		.market   .img-text   .text   .desc, .market   .img-text   .text   .name
			{
			margin: auto 0
		}
		
		.market   .img-text   .text, .market   .img-text   .text   * {
			color: #b5a396;
			text-overflow: ellipsis;
			white-space: nowrap;
			overflow: hidden
		}
		
		.market   .img-text   .text   .name {
			color: #4a4a4a;
			font-size: 24px;
			font-weight: 700;
			line-height: 28px;
			display: inline
		}
		
		.market   .img-text   .text   .market-icon {
			margin: 5px 0 0 15px;
			width: 15px;
			height: 15px;
			-o-object-fit: cover;
			object-fit: cover;
			border-radius: 7.5px
		}
		
		.market   .img-text   .text   .desc {
			color: #9b9b9b;
			font-size: 12px;
			font-weight: 300;
			height: 16px
		}
		
		.market   .hashtag {
			margin: -5px 24px 0 14px;
			max-height: 53px;
			overflow: hidden
		}
		
		.market   .hashtag   .item {
			height: 18px;
			margin: 5px 0 0 10px;
			overflow: hidden;
			display: inline-block;
			padding: 0 18px;
			color: #b5a396;
			text-align: center;
			border-radius: 10px;
			background-color: #fff
		}
		
		.market.kiosk {
			position: fixed;
			top: 0;
			margin-top: 0;
			z-index: 100;
			width: 100%
		}
		
		.takeout-select {
			position: relative;
			padding: 0px 15px;
			overflow: hidden
		}
		
		.takeout-select .select {
			height: 45px;
			display: inline-block;
			width: 40%;
			padding: 10px 0 10px 9%;
			vertical-align: middle
		}
		
		.takeout-select .select.last {
			padding: 10px 0 10px 10%;
			text-align: center
		}
		
		.takeout-select .select img {
			width: 19px;
			height: 19px;
			margin-top:10px;
			line-height: 2.1
		}
		
		.takeout-select .select .text {
			line-height: 2.1;
			margin-left: 5px;
			margin-top: 1px;
			color: #786253;
			font-weight: 600;
			font-size: 20px;
			display: inline-block;
			vertical-align: top;
			opacity: 1
		}
		
		.takeout-select   .select   .text.not-selected {
			color: #786253;
			opacity: .5
		}
		
		@media all and (max-width:450px) {
			.takeout-select   .select.last {
				padding: 10px 0 10px 5%
			}
			.takeout-select   .select   img {
				width: 17px;
				height: 17px;
				padding: 12px 0 11px
			}
			.takeout-select   .select   .text {
				font-size: 18px;
				margin-top: 2px
			}
		}
		
		.takeout-select   .separator {
			position: absolute;
			left: 15px;
			right: 15px;
			bottom: .55px;
			height: 1px;
			background-color: #b5a396
		}
		
		.green-point-select {
			position: relative;
			margin-left: -15px;
			margin-right: -15px;
			padding: 0 15px;
			overflow: hidden
		}
		
		.green-point-select   .select {
			padding: 10px 0 10px 9%;
			vertical-align: middle
		}
		
		.green-point-select   .select.last {
			padding: 10px 0 10px 10%;
			text-align: center
		}
		
		.green-point-select   .select   .keep-message {
			margin-top: -5px;
			margin-left: 3px;
			color: #b5a396;
			font-size: 15px;
			font-weight: 300
		}
		
		.green-point-select   .select   .keep-message   span {
			color: #b5a396
		}
		
		.green-point-select   .select   img {
			width: 19px;
			height: 19px;
			padding: 12px 3px 11px 0;
			line-height: 2.1
		}
		
		.green-point-select   .select   .text {
			line-height: 2.1;
			margin-left: 5px;
			margin-top: 1px;
			color: #786253;
			font-weight: 600;
			font-size: 20px;
			display: inline-block;
			vertical-align: top;
			opacity: 1
		}
		
		.green-point-select   .select   .text.not-selected {
			color: #786253;
			opacity: .5
		}
		
		@media all and (max-width:450px) {
			.green-point-select   .select.last {
				padding: 10px 0 10px 5%
			}
			.green-point-select   .select   img {
				width: 17px;
				height: 17px;
				padding: 12px 0 11px
			}
			.green-point-select   .select   .text {
				font-size: 18px;
				margin-top: 2px
			}
		}
		
		.green-point-select   .separator {
			position: absolute;
			left: 15px;
			right: 15px;
			bottom: .55px;
			height: 1px;
			background-color: #b5a396
		}
		
		.checkout-separator {
			position: relative;
			margin: 2px 0 10px;
			bottom: .55px;
			height: 1px;
			background-color: #b5a396
		}
		
		.checkout-separator.no-margin-top {
			margin: 0 0 10px
		}
		
		.phone-input {
			font-size: 12px;
			margin: 5px 0 0
		}
		
		.phone-input   .info {
			color: #000;
			visibility: hidden;
			display: none
		}
		
		.phone-input   .info   .phone-title {
			position: relative;
			top: 8px;
			height: 20px;
			margin: 10px 0 0;
			font-size: 17px;
			font-weight: 700;
			line-height: 20px;
			text-align: left
		}
		
		.phone-input   .info   .phone-option {
			position: relative;
			margin-top: 10px;
			margin-right: 110px;
			width: 100%;
			color: #b5a396;
			font-size: 15px;
			font-weight: 300
		}
		
		.phone-input   .info   .phone {
			height: 80px;
			line-height: 70px;
			text-align: center;
			font-size: 40px;
			padding-top: 30px;
			color: #786253
		}
		
		.phone-input   .info   .dial-component {
			margin-bottom: 40px;
			text-align: center
		}
		
		.phone-input   .info   .dial-component   .dial-row {
			height: 130px
		}
		
		.phone-input   .info   .dial-component   .dial-row   .dial {
			display: inline-block;
			background-color: transparent;
			padding: 10px;
			margin: 10px;
			width: 115px;
			height: 100%;
			line-height: 95px;
			text-align: center;
			font-size: 50px;
			font-weight: 300;
			border: none
		}
		
		.phone-input   .info   .dial-component   .dial-row   .dial:active:not (.no-border
			):enabled   .round {
			background-color: #786253;
			color: #f9f9f9;
			opacity: .9
		}
		
		.phone-input   .info   .dial-component   .dial-row   .dial:active:not (.no-border
			):enabled   .round.no-border {
			background-color: #fff;
			color: #786253
		}
		
		.phone-input   .info   .dial-component   .dial-row   .dial   .round {
			width: 95px;
			height: 95px;
			border-radius: 48px;
			border: .9px solid #786253;
			color: #786253
		}
		
		@media all and (max-width:450px) {
			.phone-input   .info   .phone {
				padding-top: 20px
			}
			.phone-input   .info   .dial-component   .dial-row {
				height: 110px
			}
			.phone-input   .info   .dial-component   .dial-row   .dial {
				width: 90px;
				height: 90px;
				line-height: 80px;
				margin: 2px;
				padding: 7px;
				font-size: 35px
			}
			.phone-input   .info   .dial-component   .dial-row   .dial   .round {
				width: 75px;
				height: 75px;
				border-radius: 38px
			}
		}
		
		.phone-input   .info   .dial-component   .dial-row   .dial   .round.no-border
			{
			font-size: 30px;
			font-weight: 500;
			border: none
		}
		
		@media all and (max-width:450px) {
			.phone-input   .info   .dial-component   .dial-row   .dial   .round.no-border
				{
				font-size: 25px
			}
		}
		
		.story {
			width: 600px;
			margin-right: 0px;
			float: left;
		}
		
		.story.narrow {
			width: 300.6px;
		}
		
		.cursor {
			cursor: pointer;
		}
		</style>
		<!-- css end -->
		
		<!-- modal -->
		<!-- product modal start-->
		<div class="modal" id="product">
			<div class="modal-dialog">
				<div class="modal-content" style="width: 570px; max-height: 748px;">
					<div class="balloon container" style="padding:0px;">
						<button class="close">
							<img src="${path}/resources/img/closeModal.png">
						</button>
						<div class="image-slider"
							id="image-slider" data-jssor-slider="1"
							style="width: 570px; height: 570px; visibility: visible;">
							<div style="position: absolute; display: block; top: 0px; left: 0px; width: 570px; height: 570px;">
								<div style="position: absolute; display: block; top: 0px; left: 0px; width: 570px; height: 570px; transform: scale(1.001);">
									<div data-u="slides" style="margin: 0px; padding: 0px; z-index: 0; position: absolute; pointer-events: none; top: 0px; left: 0px; width: 570px; height: 570px;">
										<div style="width: 570px; height: 570px; position: absolute; display: block; z-index: 0;"></div>
									</div>
									<div data-u="slides" style="margin: 0px; padding: 0px; z-index: 0; position: absolute; overflow: hidden; top: 0px; left: 0px; width: 570px; height: 570px;">
										<div class="" data-events="auto" data-display="block" style="z-index: 1; top: 0px; left: 0px; width: 570px; height: 570px; position: absolute; overflow: hidden; display: block;">
											<!-- w=4038&amp;h=4038 -->
											<img data-u="image" src="" 
												border="0"
												style="top: 0px; left: 0px; width: 570px; height: 570px; position: absolute; display: block; z-index: 1;"
												data-events="auto" data-display="block">
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="popup-text">
							<div class="title">
								브루다 콜드브루
							</div>
							<div class="option-name">사이즈 (필수)</div>
							<div class="segmented">
								<label>
									<input class="" type="radio" value="windows" name="o0">
									<span class="label" style="width:537px;">M Size Only</span>
								</label>
							</div>
							<div class="option-name">
								핫/아이스 (필수)
							</div>
							<div class="segmented">
								<label>
									<input type="radio" value="windows" name="o1">
									<span class="label" style="width: 260px;">Hot</span>
								</label>
								<label>
									<input type="radio" value="windows" name="o1">
									<span class="label" style="width: 260px;">Ice</span>
								</label>
							</div>
							<div class="menu-count">
								<img class="count-image" src="${path}/resources/img/menu-minus.png">
								<div class="count-modify">1 개</div>
								<img class="count-image" src="${path}/resources/img/menu-plus.png">
							</div>
							<button class="action">4,000원 담기</button>
						</div>
					</div>
				</div>
			</div>
		</div>
		<!-- product modal end-->
		<!-- cart modal start -->
		<div class="modal" id="cart">
			<div class="modal-dialog">
				<div class="modal-content" style="width: 570px; max-height: 748px;">
					<div class="popup-text">
						<div class="order name">주문</div>
						<div class="market">
							<div class="img-text">
								<div class="text">
									<div class="name">브루다 인사동점</div>
									<img  class="market-icon"
										src="">
									<div class="desc">서울특별시 종로구 우정국로 36</div>
								</div>
							</div>
						</div>
						<div class="list-header">
							주문 내역
						</div>
						<div class="list-cell clickable ">
							<div >
								<div class="title">
									<div class="name">브루다 콜드브루</div>
								</div>
								<div class="option">M Size Only⋅Hot</div>
								<div class="price">4,000원</div>
							</div>
							<img class="menu-remove"
								src="${path}/resources/img/menuDelete.png">
						</div>
						<div 
							class="list-header short-margin-top">추가 요청</div>
						<div class="list-cell clickable ">
							<div class="title-only ">메모</div>
							<img class="accessory square" src="">
						</div>
						<div class="list-header no-margin-top takeout-info" id="takeout-info">
							이용 방식 (테이크아웃 매장)
						</div>
						<div class="takeout-select">
							<div class="select">
								<img src="${path}/resources/img/cellSelected2.png">
								<div class="text takeout-text">테이크아웃</div>
							</div>
							<div class="select last">
								<img src="${path}/resources/img/cellEnabled.png">
								<div class="text takeout-text not-selected">매장이용</div>
							</div>
						</div>
						<div class="list-notice">
							예상 대기 시간: 5분
						</div>
						<div class="checkout-separator"></div>
						<div class="list-notice list-margin">
							<span style="vertical-align: middle;">*</span>
							주문 접수와 동시에 메뉴가 준비되므로 
							<span class="order-disable">결제 후 주문 변경 및 환불이 어렵습니다.</span>
						</div>
						<button class="action">
							<span class="">4,000원 결제</span>
						</button>
					</div>
				</div>
			</div>
		</div>
		<!-- cart modal end -->
	</head>
<body>
	<div class="container-fluid">
		<div class="navbar row border">
			<div class="col-1">
				<img class="logo cursor" src="${path }/resources/img/mermaid.png" style="width:80px;height:80px;"
					onclick="location.href='${path}'">
			</div>
			<div class="col-1 opencart count ml-auto">
				<svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 512 512" style="width:50px;height:50px;">
				  <g>
				    <g>
				      <path d="m464.5,301.1l36.5-178h-359.7l-12.5-59.2-108.4-52.9-9.4,18.7 99,47.8 50,238.8h289c0,0 28.5,17.9 17.5,40.5-4.9,7-12.5,15.6-26.1,15.6h-287.6v20.6h287.7c19.8,0 36.5-10.4 45.9-27 18.4-34.4-21.9-64.9-21.9-64.9zm-286.7-5.7l-32.3-151.6h330.5l-31.3,151.6h-266.9z"/>
				      <path d="m212.2,422.1c-21.9,0-39.6,17.6-39.6,39.4s17.7,39.4 39.6,39.4 39.6-17.6 39.6-39.4-17.7-39.4-39.6-39.4zm0,58.1c-10.4,0-18.8-8.3-18.8-18.7s8.3-18.7 18.8-18.7 18.8,8.3 18.8,18.7-8.4,18.7-18.8,18.7z"/>
				      <path d="m424.9,422.1c-21.9,0-39.6,17.6-39.6,39.4s17.7,39.5 39.6,39.5 40.7-17.6 39.6-39.4c0-21.8-17.7-39.5-39.6-39.5zm18.8,39.5c0,10.4-8.3,18.7-18.8,18.7s-18.8-8.3-18.8-18.7 8.3-18.7 18.8-18.7 19.8,8.3 18.8,18.7z"/>
				    </g>
				  </g>
				</svg>
			</div>
			<div class="col-1">
				<img class="" src="${path}/resources/img/btn_mypage.png" style="width:60px;height:60px;"
					onclick="location.href='${path}/owner';">
			</div>
		</div>
		<div class="market-nav row border" style="padding-bottom: 15px; background-color: #04A967;">
			<div class="img-text">
				<div class="market-nav-info">
					<div class="market-nav-image">
						<img class="cursor" src="${path}/resources/img/index.jpg" alt="커피점로고">
					</div>
					<div class="text">
						<div class="name">인어들</div>
						<div class="desc">카페</div>
					</div>
				</div>
				<!-- <div class="market-nav-select cursor">매장변경 &gt;</div> -->
			</div>
		</div>
		<div class="row border search">
		    <select class="col-3" name="searchType">
		      <%-- <option value="n"<c:out value="${scri.searchType == null ? 'selected' : ''}"/>>-----</option>
		      <option value="t"<c:out value="${scri.searchType eq 't' ? 'selected' : ''}"/>>점포</option>
		      <option value="t"<c:out value="${scri.searchType eq 't' ? 'selected' : ''}"/>></option> --%>
		      <option value="nonselect"<c:out value="" />>-----</option>
		      <!-- 소유하는 점포 숫자만큼 반복문 -->
		      <option value="store"<c:out value="" />>점포1</option>
		      <option value="store"<c:out value="" />>점포2</option>
		    </select>
		    <input class="col-6" type="text" name="keyword" id="keywordInput" value=""/>
		    <button class="col-2" id="searchBtn" type="button">검색</button>
			
		</div>
		<div class="stories component row border">
			<div class="story component menu single-column component" id="">
				<div class="title component">COFFEE</div>
				<div class="menu-item index 01">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							커피 1
						</div>
						<div class="price">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item index 02">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							커피 2
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item index 03">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							커피 3
						</div>
						<div class="price">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item index 04">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							커피 4
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="show-more">
					<div class="text">더보기</div>
				</div>
			</div>
			<div class="story component menu single-column component" id="">
				<div class="title component">COFFEE</div>
				<div class="menu-item index 05">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							커피 1
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item index 06">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							커피 2
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item index 07">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							커피 3
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
			</div>
			<div class="story component menu single-column component" id="">
				<div class="title component">COFFEE</div>
				<div class="menu-item">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							더미 1
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							더미 2
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							더미 3
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							더미 4
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="show-more">
					<div class="text">더보기</div>
				</div>
			</div>
			<div class="story component menu single-column component" id="">
				<div class="title component">COFFEE</div>
				<div class="menu-item">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							더미 1
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							더미 2
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							더미 3
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="menu-item">
					<img
						src="${path}/resources/img/product.jpg">
					<div class="text">
						<div class="name name-only">
							더미 4
						</div>
						<div class="price ">4,000원</div>
					</div>
					<button class="add">
						<img src="${path}/resources/img/add.png">
					</button>
					<div class="divider"></div>
				</div>
				<div class="show-more">
					<div class="text">더보기</div>
				</div>
			</div>
		</div>
		<div class="company row border">
			<div class="name col-10">주식회사 머메이드오더</div>
			<div class="col-2 close_more" style="cursor:pointer;">더보기</div>
			<div class="introduce col-12">
				<div class="info row" style="display:none;">
					대표이사 : 신재원<br> 사업자등록번호 :
					123-45-78912<br> 통신판매업 :
					제 2020-서울강남-1234<br> 메일
					: abcd@efg.com<br>
					고객센터 : 010-1111-1123<br>
					주소 : 서울특별시<br> 호스팅서비스제공자 : 주식회사 머메이드오더<br>
					<br>
				</div>
				<div class="info2 row">
					머메이드오더는 통신판매중개자이며 통신판매의 당사자가 아닙니다.<br>
					머메이드오더는 상품거래 정보 및 거래에 대한 책임을 지지 않습니다.
				</div>
			</div>
		</div>
	</div>
	<script>
		$(function () {
			$(".company.row").children('.col-2').click(function() {
				if ($(".company.row").children('.col-2').hasClass("close_more")) {
					$(".company.row").children('.col-2').removeClass("close_more").addClass("open_more");
					$(this).text('접기');
					$(".info.row").attr("style", "display:block");
				} else {
					$(".company.row").children('.col-2').removeClass("open_more").addClass("close_more");
					$(this).text('더보기');
					$(".info.row").attr("style", "display:none");
				}
			});
			$(".market-select:first").click(function() {
				alert('등록된 매장으로 변경');
			});
			$(".show-more:first").click(function() {
				alert('4개 이상의 상품이 있을 경우 modal창으로 카테고리 모든 메뉴 보여주기');
			});
			$(".market-image>img").click(function() {
				alert('가게 소개 modal');
			});
		});
		document.querySelectorAll(".index").forEach(item => {
		  item.addEventListener('click', event => {
			$("#product").modal('show');	    
		  });
		});
		document.querySelectorAll(".opencart").forEach(item => {
		  item.addEventListener('click', event => {
			$("#cart").modal('show');	    
		  });
		});
		document.querySelector(".close").addEventListener('click',()=>{
			$("#product").modal('hide');
			$("#cart").modal('hide');
		});
	</script>
</body>

</html>